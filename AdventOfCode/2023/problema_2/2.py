def getSets(string):
    ...
    for ss in sets:
        ...
        for sss in ss.split(','):
            if('red' in sss):
                red += int(sss[:sss.find('red')])
            if('blue' in sss):
                blue += int(sss[:sss.find('blue')])
            if('green' in sss):
                green += int(sss[:sss.find('green')])
        if(red>12 or green >13 or blue >14):
            return False
    return True
def getMinForSet(string):
    sets = string.split(';')
    red = green = blue = 1
    for ss in sets:
        for sss in ss.split(','):
            if('red' in sss):
                val=int(sss[:sss.find('red')])
                if(val >red):
                    red = val
            if('blue' in sss):
                val= int(sss[:sss.find('blue')])
                if(val>blue ):
                    blue=val
            if('green' in sss):
                val= int(sss[:sss.find('green')])
                if(val>green):
                    green = val
    return red*green*blue


text_1='''Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green'''

text ='''Game 1: 7 red, 8 blue; 6 blue, 6 red, 2 green; 2 red, 6 green, 8 blue; 9 green, 2 red, 4 blue; 6 blue, 4 green
...
for s in splitted:
    ss=s[s.find(':')+1:].replace(' ','')
    if(getSets(ss)):
        out+=i
    out_2+=getMinForSet(ss)
    i+=1
